...
import ServicesLicensingIcon, {
  SERVICES_LICENSING_ICON_OPTIONS,
  servicesLicensingIconLabel,
} from "@/components/icons/ServicesLicensingIcon";
...
export default function ServicesLicensingIconPicker({ value, onChange }: Props) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const selected = typeof value === "string" && value ? value : "blocks";
  const selectedLabel = servicesLicensingIconLabel(selected);

  const filteredOptions = useMemo(() => {
    const query = search.trim().toLowerCase();
    if (!query) return SERVICES_LICENSING_ICON_OPTIONS;
    return SERVICES_LICENSING_ICON_OPTIONS.filter(
      (option) =>
